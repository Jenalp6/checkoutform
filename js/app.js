...
function adding(item) {
  ...
  payment.innerText =
    "$" +
    (
      parseFloat(payment.innerText.slice(1)) + parseFloat(price.slice(1))
    ).toFixed(2);
  if (amount != 0) {
...
function destroy(item) {
  let price =
    this.parentElement.firstElementChild.nextElementSibling.firstElementChild.innerText.slice(
      1
    );
  let amount =
    this.previousElementSibling.firstElementChild.nextElementSibling.innerText;
  payment.innerText =
    "$" + (parseFloat(payment.innerText.slice(1)) - price * amount).toFixed(2);
